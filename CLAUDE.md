...
Скилы лежат в `.devin/skills/`. **Вызывать через `skill invoke <name>`** по ключевым словам пользователя.

| Скил | Файл | Триггеры | Что делает |
|---|---|---|---|
| `run-app` | `.devin/skills/run-app/SKILL.md` | «запусти», «запусти приложение», «открой через туннель», «проверь» | Убивает порт 3000 → `npm start` в фоне → ждёт URL туннеля → smoke-test `/api/has-kids` |
| `commit` | `.devin/skills/commit/SKILL.md` | «закоммить», «сделай коммит», «сохрани изменения» | `git diff` → генерирует сообщение → спрашивает подтверждение → `git add -A && git commit` |
| `push` | `.devin/skills/push/SKILL.md` | «запушь», «отправь на GitHub», «push» | Проверяет незакоммиченные изменения → `git push origin master` |
